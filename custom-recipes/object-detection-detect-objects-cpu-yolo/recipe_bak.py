...
# 이미지 경로를 배치 단위로 나누어 처리
for i in range(0, len(paths), batch_size):
    batch_paths = paths[i:i + batch_size]
    batch_paths = list(map(lambda x: op.join(folder_path, x[1:]), batch_paths))  # 전체 경로로 변환
    ...
    for batch_i in range(boxes.shape[0]):
        cur_path = [batch_paths[batch_i].split('/')[-1]]  # 현재 이미지의 파일 이름

        at_least_one = False  # 적어도 하나의 객체가 탐지되었는지 여부

        # 탐지된 각 객체에 대해 처리
        for box, score, label in zip(boxes[batch_i], scores[batch_i], labels[batch_i]):
            if score < confidence: break  # 신뢰도가 임계값보다 낮으면 종료 (점수는 내림차순 정렬)

            at_least_one = True  # 객체가 탐지되었음을 표시

            int_box = list(box.astype(int))  # 바운딩 박스 좌표를 정수로 변환
            label_name = labels_to_names[label]  # 라벨 이름을 가져옴
        ...
        if not at_least_one and configs['record_missing']:
            df.loc[df_idx] = cur_path + [np.nan for _ in range(6)]  # 누락된 경우 NaN으로 채움
            df_idx += 1
